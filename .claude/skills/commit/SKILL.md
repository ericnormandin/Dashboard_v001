---
name: commit
description: Stage and commit all changes locally. Runs ruff, stages everything, commits with an auto-generated message. Does NOT push to remote.
---

Stage and commit all current changes locally. Always auto-generate the commit message — never ask the user for one. Do NOT push.
...
If ruff check reports errors, show them to the user and stop. Ask them to fix the errors before committing.
...
- A reminder that changes are local only (not pushed)
