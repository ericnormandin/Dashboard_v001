...
// ================= STYLING HELPERS =================
// (formerly part of crypto screen; kept for getAssetIcon/getAssetBgColor reuse)
function _unusedCryptoOld() {
    const feedContainer = document.getElementById('crypto-ticker-feed');
    const balancesGrid = document.getElementById('crypto-balances-grid');
    if (!feedContainer || !balancesGrid) return;
    
    feedContainer.innerHTML = `<div class="py-6 text-center text-terminal-muted"><i class="fa-solid fa-spinner animate-spin"></i> QUERYING KRAKEN NODES...</div>`;
    balancesGrid.innerHTML = `<div class="col-span-4 py-6 text-center text-terminal-muted"><i class="fa-solid fa-spinner animate-spin"></i> RETRIEVING ACCOUNT LEDGER...</div>`;

    const pairs = ['BTC/USD', 'ETH/USD', 'SOL/USD', 'ADA/USD'];
    let fetchedPrices = { ...currentPrices };
    
    // 1. Fetch live or fallback prices for Kraken ticker feeds
    feedContainer.innerHTML = '';
    for (const pair of pairs) {
        const asset = pair.split('/')[0];
        try {
            const res = await fetch(`${API_BASE_URL}/api/kraken/ticker/${encodeURIComponent(pair)}`);
            if (!res.ok) throw new Error();
            const data = await res.json();
            
            fetchedPrices[asset] = data.last;
            currentPrices[asset] = data.last; // cache
            
            const isPositive = data.change_24h >= 0;
            const item = document.createElement('div');
            item.className = 'border border-terminal-border p-3.5 rounded bg-[#131215] hover:border-terminal-bronze/40 transition duration-300 flex justify-between items-center select-none';
            item.innerHTML = `
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded ${getAssetBgColor(asset)} flex items-center justify-center text-slate-100 text-xs">
                        <i class="${getAssetIcon(asset)}"></i>
                    </div>
                    <div>
                        <div class="font-bold text-slate-200">${asset} / USD</div>
                        <div class="text-[9px] text-terminal-muted tracking-widest mt-0.5">FEED: Kraken API</div>
                    </div>
                </div>
                <div class="text-right">
                    <div class="font-bold text-slate-200 font-sans text-sm">$${data.last.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:4})}</div>
                    <div class="text-[9px] ${isPositive ? 'text-terminal-green' : 'text-terminal-red'} font-sans font-semibold mt-1">
            ...
            // Offline mock renderer inside list
            const mockRate = currentPrices[asset];
            const changePercent = asset === 'BTC' ? 1.85 : asset === 'SOL' ? 4.12 : asset === 'ETH' ? -0.45 : 0.50;
            ...
            item.innerHTML = `
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="font-bold text-slate-200">${asset} / USD</div>
                        <div class="text-[9px] text-terminal-bronze tracking-widest mt-0.5">FEED: Offline Mock</div>
                    </div>
                </div>
                <div class="text-right">
                    <div class="font-bold text-slate-200 font-sans text-sm">$${mockRate.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:4})}</div>
                    <div class="text-[9px] ${isPositive ? 'text-terminal-green' : 'text-terminal-red'} font-sans font-semibold mt-1">
    ...
    // 2. Fetch balances (live or fallback)
    try {
        const res = await fetch(`${API_BASE_URL}/api/kraken/balance`);
        if (!res.ok) throw new Error();
        const data = await res.json();
        renderCryptoBalances(data.free, fetchedPrices);
    } catch (err) {
        // Fallback simulated local balances
        ...
        renderCryptoBalances(mockBalanceData, fetchedPrices);
    }
}

function renderCryptoBalances(assetsList, priceMap) {
    const balancesGrid = document.getElementById('crypto-balances-grid');
    ...
    for (const [asset, amount] of Object.entries(assetsList)) {
        const price = priceMap[asset] || 1.00;
        const val = amount * price;
        totalCryptoValue += val;
        
        const col = document.createElement('div');
        col.className = 'border border-terminal-border p-4 rounded bg-[#131215] hover:border-terminal-bronze/40 transition duration-300 font-mono select-none relative group overflow-hidden';
        col.innerHTML = `
            <div class="flex items-center justify-between mb-3.5 border-b border-terminal-border pb-2">
                <span class="font-bold text-slate-200 uppercase text-[9px] tracking-widest">${asset} LEDGER</span>
                <div class="w-6 h-6 rounded ${getAssetBgColor(asset)} flex items-center justify-center text-slate-100 text-[10px]">
                    <i class="${getAssetIcon(asset)}"></i>
                </div>
            </div>
            <div class="text-[9px] text-terminal-muted">Liquid Bal.</div>
            <div class="text-sm font-bold text-slate-200 mt-0.5 font-sans">${amount.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 6})}</div>
            <div class="text-[10px] font-bold text-terminal-gold mt-1.5 font-sans">$${val.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})}</div>
        `;
        balancesGrid.appendChild(col);
    }

    // Refresh dynamic Overview Net Worth metric
    const overviewCryptoVal = document.getElementById('overview-crypto-val');
    if (overviewCryptoVal) {
        overviewCryptoVal.textContent = `$${totalCryptoValue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
    }
    
    // Estimated net worth aggregation: crypto total ($23,892.20 equivalent) + regular accounts
    const overviewNetWorth = document.getElementById('overview-net-worth');
    if (overviewNetWorth) {
        const aggregatedTotal = totalCryptoValue + 34800.00 + 12450.75 + 1790.73; // TFSA + RRSP + Cashflow balances
        overviewNetWorth.textContent = `$${aggregatedTotal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
    }
}
